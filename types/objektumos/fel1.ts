...
function aranagyobbTizezer(termekek:{nev:string,ar:number}[]):{nev:string,ar:number}[]{
    const dragabb=termekek.filter((termek)=>termek.ar >10000);
    return dragabb;
...
function elsoSzamj(n:number):number{
    const szamma=n.toString().split('').map(Number);
...
const szavaktomb:string[]=['asztal', 'programozás', 'tanul', 'gépezet'];


function hosszabbMintHat(szavaktomb:string[]):string[]{

    const hossza=szavaktomb.filter((szo)=>szo.length> 6);
    return hossza;
