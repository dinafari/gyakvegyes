...
var tomb1 = [1, 2, 2, 3, 2];
function hanyszorSzam(tomb1, keresett) {
    var hanyszor = tomb1.filter(function (szam) { return szam === keresett; }).length;
    return hanyszor;
}
...
function palindromE(n) {
    var eredeti = n.toString();
    var forditott = eredeti.split('').reverse().join('');
...
//Pl.: 3 és 7 → 4+5+6 = 15
function kozottiOszeg(a, b) {
    var legn = Math.max(a, b);
    var legk = Math.min(a, b);
    var osszeguk = 0;
    for (var i = legk + 1; i < legn; i++) {
        osszeguk += i;
    }
    return osszeguk;
}
