...
function ketszSzorzUtszamj(a:number,b:number):number{
    const szorzatuk=a*b;
...
const tomb1:number[]=[3, 5, 6, 9, 11, 15];

function oszthDBHarom(tomb1:number[]):number{

    const oszthdbszam= tomb1.filter(szam=>szam % 3 ===0).length;
    return oszthdbszam;
...
function legnKul(x:number,y:number,z:number):number{

    
    const kul1= Math.abs(x-y);
    const kul2= Math.abs(x-z);
    const kul3= Math.abs(y-z);
